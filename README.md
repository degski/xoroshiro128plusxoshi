...
The actual implementation tested deviates slightly (see [code](https://github.com/degski/xoroshiro128plusxoshi/blob/master/xoroshiro128plusxoshi.hpp)) from the above (order of operations), but my guess is that it all don't matter after optimization. Without knowing the final results of testing, I suspect that failure can be postponed by repeating (as many times as required) the `result = ( ( result >> 32 ) ^ result ) * itype { 0x1AEC805299990163 };` line. The `magic number` has to have a modular multiplicative inverse (as to not end up with potential short cycles). This extra line(s) affect the speed (but how much?). More searching also has to go into [finding](https://github.com/degski/inthashing) a better `magic constant` (which I know exists). I've used as a method for finding a better `magic constant` a combination of evolutionary programming and iterative deepening. The potential improvement will come at **zero cost** as compared to the current version. As the 'final mixer' is (part of) a reversible hash function, jump functions will be trivial to implement, taking any `xoroshiro128plus` jump function and hash that result.
...
4TB barrier broken! testing (11hrs till 8TB) ... 
